...
import orev3.experiments.rq003_experiment4 as experiment4_module
from orev3.experiments.rq003_experiment4 import (
...
def test_average_rank_ties_and_predeclared_directions() -> None:
    values = [CanonicalRational.make(0)] * 25
    values[0] = CanonicalRational.make(10, 3)
    values[1] = CanonicalRational.make(20, 6)
    values[2] = CanonicalRational.make(5, 2)
    ...
    assert _average_ranks(
        [CanonicalRational.make(0)] * 25, descending=True
    ) == (13.0,) * 25
...
def test_protocol_document_identity_is_current() -> None:
    ...
    protocol = root / (
        ...
        "rq003-experiment-004-deployment-per-miner-predictive-evaluation.md"
    )
...
def test_end_to_end_freezes_rankings_before_outcome_join(
    ...
    monkeypatch.setattr(experiment4_module, "EXPERIMENT4_BOOTSTRAP_REPLICATES", 20)
    _bind_frozen_test_source(monkeypatch)

    result = execute_experiment4(_configuration(dataset, output))
    ...
    assert tuple(sorted(path.name for path in output.iterdir())) == tuple(
        sorted(EXPERIMENT4_ARTIFACT_NAMES)
    )
    ...
    assert "winning_square" not in ranking_text
    assert all(
        set(candidate).isdisjoint({"deployed_lamports", "miner_count"})
        and "deployment_per_miner" in candidate
        for record in ranking_records
    ...
    assert all(
        record["deployment_per_miner_definition_identity"]
        == DEPLOYMENT_PER_MINER_DEFINITION_IDENTITY
        for record in ranking_records
    )
    round_11 = next(record for record in ranking_records if record["round_id"] == 11)
    assert round_11["candidates"][23]["primary_average_rank"] == 12.0
    assert round_11["candidates"][24]["primary_average_rank"] == 12.0
    assert round_11["candidates"][23]["primary_tie_group_size"] == 3
    assert round_11["candidates"][24]["primary_tie_group_size"] == 3
    ...
    assert conformance["feature_set_ordered_fields"] == ["deployment_per_miner"]
    assert (
        conformance["measurement_definition_identity"]
        == DEPLOYMENT_PER_MINER_DEFINITION_IDENTITY
    )
...
def test_final_validation_accepts_external_source_bytes_and_keeps_generated_strict(
    ...
    result = execute_experiment4(_configuration(dataset, output))
    assert validate_experiment4_artifacts(
        output,
...
def test_regeneration_is_byte_deterministic(
    ...
    first = execute_experiment4(_configuration(dataset, tmp_path / "first"))
    second = execute_experiment4(_configuration(dataset, tmp_path / "second"))
...
experiment4_module._bind_execution_source = lambda configuration: (
    SourceCommitProvenance(
        "a" * 40,
...
def test_positive_deployment_with_zero_miners_is_excluded_before_ranking(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    dataset = _write_dataset(tmp_path, invalid_ratio_round=15)
    output = tmp_path / "output"
    monkeypatch.setattr(experiment4_module, "EXPERIMENT4_BOOTSTRAP_REPLICATES", 20)
    _bind_frozen_test_source(monkeypatch)
    ...
    assert result.replay_rounds == 6
    assert result.ranked_decisions == 5
    lifecycle = _load_json(output / "lifecycle.json")
    assert lifecycle["selection_counts"] == {
        "ranked": 5,
        "undefined_deployment_per_miner": 1,
    }
    rankings = _load_ranking_artifact(output / "ranking_artifact.jsonl")
    assert 15 not in {record["round_id"] for record in rankings}


def test_ranking_validation_rejects_noncanonical_rational(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    dataset = _write_dataset(tmp_path)
    output = tmp_path / "output"
    monkeypatch.setattr(experiment4_module, "EXPERIMENT4_BOOTSTRAP_REPLICATES", 20)
    _bind_frozen_test_source(monkeypatch)
    execute_experiment4(_configuration(dataset, output))
    record = _load_ranking_artifact(output / "ranking_artifact.jsonl")[0]
    record["candidates"][0]["deployment_per_miner"] = {
        "denominator": 2,
        "numerator": 2,
        "rational_identity": "0" * 64,
    }
...
def test_phase_scope_has_no_strategy_model_or_economics_dependency() -> None:
    tree = ast.parse(inspect.getsource(experiment4_module))
    imported_modules = {
        node.module
...
def _configuration(dataset: Path, output: Path) -> Experiment4Configuration:
    return Experiment4Configuration(
        replay_dataset_path=dataset,
        output_directory=output,
...
def _write_dataset(
    tmp_path: Path, *, invalid_ratio_round: int | None = None
) -> Path:
    ...
    for round_id, coverage, source, capture_mode, winner in settings:
        ...
        for observation_index, slots_remaining in enumerate((10, 5)):
            ...
            miner = [square + 1 for square in range(25)]
            deployed = [100 + square for square in range(25)]
            if round_id == 11:
                deployed[23] = 240
                deployed[24] = 250
            if round_id == invalid_ratio_round:
                miner[0] = 0
            raw_records.append(
                {
                    "board": {
