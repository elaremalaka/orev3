...
def test_phase_scope_has_no_strategy_model_or_economics_dependency() -> None:
    tree = ast.parse(inspect.getsource(experiment1_module))
    imported_modules = {
        node.module
...
def _configuration(dataset: Path, output: Path) -> Experiment1Configuration:
    return Experiment1Configuration(
        replay_dataset_path=dataset,
        output_directory=output,
...
def _write_dataset(tmp_path: Path) -> Path:
    ...
    for round_id, coverage, source, capture_mode, winner in settings:
        ...
        for observation_index, slots_remaining in enumerate((10, 5)):
            ...
            deployed = [square for square in range(25)]
            if round_id == 11:
                deployed[23] = deployed[24]
            raw_records.append(
                {
                    "board": {
                        "end_slot": end_slot,
