...
def test_marker_preflight_and_atomic_creation_are_separately_authorized(
    tmp_path: Path, config: RFC008Config, monkeypatch
) -> None:
    ...
    with pytest.raises(PermissionError):
        create_marker(
            ...
            source_identities=("/tmp/source|1|0|0",),
    ...
    created = create_marker(
        ...
        latest_preholdout_round_id=345999,
        source_identities=("/tmp/source|1|0|0",),
        authorization_token=MARKER_AUTHORIZATION,
    )
    assert created.first_eligible_round_id == 346000
...
def test_finalized_label_record_cannot_become_decision_input(
    ...
    assert value.count("finalized_outcomes") == 1
